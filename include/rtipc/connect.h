 ...
 * @param attr Configuration for the channel vector.
 *
 * @return A newly created and initialized vector on success, or NULL if the
 *         connection fails or is rejected by the server.
 */
ri_group_t* ri_client_socket_connect(int socket, const ri_group_attr_t *attr);
 ...
 */
ri_group_t* ri_client_connect(const char *path, const ri_group_attr_t *attr);
