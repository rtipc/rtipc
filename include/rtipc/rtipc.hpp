...
template <typename T>
class Consumer final : private ConsumerBase {
  friend class ChannelGroup;
...
template <typename T>
class Producer final : protected ProducerBase {
  friend class ChannelGroup;
...
public:
  // deserializw
  explicit ChannelGroup(const std::span<std::byte>& req,
                        std::span<int>& fds);
  ...
  template <typename T>
  Consumer<T> acquire_consumer(unsigned index) {
    size_t size = consumer_message_size(index);

    if (size < sizeof(T))
      throw std::length_error(
          "channel message size too small to hold struct");

    return Consumer<T>(acquire_consumer_impl(index));
  }

  template <typename T>
  Producer<T> acquire_producer(unsigned index) {
